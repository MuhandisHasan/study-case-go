...
func (a *UUIDArray) Scan(value interface{}) error {
	...
	*a = make(UUIDArray, 0, len(parts))
	...
	for _, s := range parts {
		s = strings.TrimSpace(strings.Trim(s, `"`)) // Menghapus space dan "
...
/* We don't use pointer here because we do not need to change the real variable. We'll create a new variable of the formatted UUIDArray that matches the Postgresql format.
 */
